...
bool Camera::Create(const Name& name)
{
	...
	m_engine->Get<Input>()->AddAction("camera_left", SDL_SCANCODE_A);
	m_engine->Get<Input>()->AddAction("camera_backward", SDL_SCANCODE_S);
	m_engine->Get<Input>()->AddAction("camera_right", SDL_SCANCODE_D);
...
{
	glm::vec3 rotation(0);
	rotation.x = m_engine->Get<Input>()->GetAxisRelative("camera_x") * 0.005;
	rotation.y = m_engine->Get<Input>()->GetAxisRelative("camera_y") * 0.005;
