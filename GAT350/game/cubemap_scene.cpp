...
bool CubemapScene::Create(const Name& name)
{
...

	{
		...
		material->textures.push_back(m_engine->Resources()->Get<Texture>("textures/uvgrid.jpg"));
...

	{
		...
		model->m_mesh->m_material = m_engine->Resources()->Get<Material>("debug_material");
...
void CubemapScene::Update()
{
	...
	model->m_shader->Use();
	model->m_shader->SetUniform("mix_value", m_mix);
	model->m_shader->SetUniform("index", m_index);

	// gui
	GUI::Update(m_engine->GetEvent());
	GUI::Begin(m_engine->Get<Renderer>());
	m_engine->Get<Editor>()->UpdateGUI();
	ImGui::SliderFloat("Index", (float*)&m_index, 0.0f, 3.0f);
	ImGui::SliderFloat("Mix", (float*)&m_mix, 0.0f, 1.0f);
